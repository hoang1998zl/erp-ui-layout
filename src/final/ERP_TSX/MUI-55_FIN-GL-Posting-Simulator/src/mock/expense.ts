...
// --- Posting helpers for FIN-11 ---
export function isPosted(d: any): boolean { return !!(d.posted_at || d.posting_id); }
export function markPosted(id: string, posting_id: string){
  const arr = listDrafts(); const d = arr.find(x => x.id===id); if (!d) return;
  (d as any).posted_at = new Date().toISOString(); (d as any).posting_id = posting_id;
  saveDrafts(arr);
}
export function listApprovedUnposted(): ExpenseDraft[] {
  return listDrafts().filter((d:any)=> d.status==='approved' && !isPosted(d));
}
