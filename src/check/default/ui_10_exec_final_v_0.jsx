 ...
 * - Auto-route per v0.4: Exec Global-first, switch to Cell if recent cell interaction (< RECENT_MS)
 * - Storage key unified: erp_ui10_exec_chatStore_v1
 * - Thread IDs unified (align with Shell tag): thread:global:r1 | thread:cell:<companyId>:<deptId>:r1
 * - Temp-user guard (probation/intern/external) blocks Global; fallback My Work (simulated)
 * - Added approval thresholds tooltip (Exec: 1B/single, 5B/month)
 * - Self-tests E1–E14 (matching CEO suite, adapted for r1) — added E13–E14 to prevent icon & routing regressions
 */

// -------------------- Config --------------------
const VERSION = "v0.4";
const RECENT_MS = 120_000; // 120s per Spec v0.4
const STORAGE_KEY = "erp_ui10_exec_chatStore_v1";
...
const seedOrg = [
  {
    ...
    children: [
      ...
      {
        id: "co3",
        label: "Đại Tín Services",
        type: "company",
        children: [
          { id: "br2", label: "Chi nhánh Bình Thạnh", type: "branch", children: [] },
        ],
      },
    ],
  },
...
function threadIdCell(companyId, deptId) {
  return `thread:cell:${companyId}:${deptId}:r1`;
}
function threadIdGlobal() {
  return `thread:global:r1`;
}

/**
 * Auto-route per v0.4
 * - Exec (level 1): default GLOBAL, switch to CELL if selected & recent (<RECENT_MS)
...
function ChatModal({ open, onClose, scopeLabel, messages, onSend, onUpload }) {
  const prompts = [
    "Tổng hợp KPI BU",
    "Top phiếu chờ duyệt của BU",
    "Rủi ro các công ty được phân công",
    "So sánh ngân sách Q3",
  ];
  if (!open) return null;
  return (
...
function runSelfTests({ companies, depts, matrix, route, chatStore }) {
  ...
  // E1: Matrix shape and value bounds
  let pass1 = true;
  let msg1 = "";
  ...
  const ok4 = gid === "thread:global:r1" && cid === "thread:cell:co1:hr:r1";
  results.push({ name: "Thread ID format (r1)", pass: ok4, detail: ok4 ? "OK" : `gid=${gid} cid=${cid}` });

  // E5: Store isolation (cell vs global)
  ...
  // E7: Exec routing recent vs stale
  ...
  results.push({ name: "Routing Exec (recent vs stale)", pass: ok7, detail: ok7 ? "OK" : `recent=${rRecent.scope} stale=${rStale.scope}` });

  // E8: Global thread id stays constant
  const rGlobal1 = route({ selected: null, lastCellTs: 0, isTemp: false });
  const rGlobal2 = route({ selected: null, lastCellTs: 0, isTemp: false });
  const ok8 = rGlobal1.id === rGlobal2.id && rGlobal1.id === "thread:global:r1";
  results.push({ name: "Global thread id stable", pass: ok8, detail: ok8 ? "OK" : rGlobal1.id + " vs " + rGlobal2.id });

  // E9: Temp-user guard blocks global
  ...
  // E10: STORAGE_KEY sanity
  const ok10 = STORAGE_KEY === "erp_ui10_exec_chatStore_v1";
  results.push({ name: "Storage key", pass: ok10, detail: ok10 ? "OK" : STORAGE_KEY });

  // E11: RECENT_MS sanity
  const ok11 = RECENT_MS === 120000;
  results.push({ name: "RECENT_MS = 120000", pass: ok11, detail: ok11 ? "OK" : String(RECENT_MS) });

  // E12: Default route is global when not temp & no selection
  const def = route({ selected: null, lastCellTs: 0, isTemp: false });
  const ok12 = def.scope === "global" && def.id === "thread:global:r1";
  results.push({ name: "Default route (global)", pass: ok12, detail: ok12 ? "OK" : `${def.scope}:${def.id}` });

  // E13: Dynamic icon rendering must not use JSX with arbitrary expressions
  try {
  ...
  // E14: Routing to cell when selection is very recent (<100ms)
  const rVeryRecent = route({ selected: sel, lastCellTs: Date.now(), isTemp: false });
  results.push({ name: "Routing: very recent -> cell", pass: rVeryRecent.scope === "cell", detail: rVeryRecent.scope });

  return results;
}

// -------------------- Main App (UI 10 – Exec) --------------------
export default function UI10Exec() {
  const [portfolio, setPortfolio] = useState(["co1", "co3"]); // công ty thuộc BU
  ...
  const [tab, setTab] = useState("overview"); // overview | matrix | approvals | budget
  ...
  // Simulate employment status (Exec có thể là temp trong thử nghiệm)
  ...
  function Overview() {
    return (
      <motion.div key="ov" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {["Doanh thu BU", "Lợi nhuận BU", "Tỷ lệ hoàn thành dự án", "AR/AP", "Burn rate", "Rủi ro BU"].map((k, i) => (
  ...
  function Budget() {
    return (
      <motion.div key="bd" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-3">
        <div className="flex items-center gap-1 text-xs text-neutral-500"><Info className="h-3.5 w-3.5" />Ngưỡng phê duyệt (v0.4): Exec ≤ <b>1 tỷ/phiếu</b>, ≤ <b>5 tỷ/tháng</b>. Trên ngưỡng → escalate.</div>
          ...
          <div key={c.id} className="flex items-center justify-between rounded-2xl border border-neutral-200 p-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-500">Ngân sách Q3: {(50 + i * 10).toLocaleString()} tỷ · Đã dùng: {(30 + i * 8).toLocaleString()} tỷ</div>
              </div>
            </div>
            <button title="Xem chi tiết ngân sách" className="rounded-xl border px-3 py-1.5 text-sm hover:bg-neutral-100" onClick={() => alert(`Mở ngân sách của ${c.label}`)}>
              Xem
            </button>
          </div>
        ))}
  ...
  function Approvals() {
    const tooltip = "Exec: 1B/single, 5B/month · Trên ngưỡng sẽ chuyển CEO/Board";
    return (
          ...
          <div key={i} className="flex items-center justify-between rounded-2xl border border-neutral-200 p-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-500">Có {Math.floor(Math.random()*5)+1} phiếu chờ duyệt (BU)</div>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-[1400px] px-4">
          <div className="flex h-14 items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <Pill>UI 10 · Exec · Final {VERSION}</Pill>
              <div className="hidden md:flex items-center gap-2">
                <Pill>BU Portfolio</Pill>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 rounded-full border border-neutral-200 px-2 py-1.5">
                ...
                <span className="text-sm">Anh (Exec)</span>
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-4 grid grid-cols-[280px,1fr,360px] gap-4">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-neutral-200 bg-white p-4">
            <SectionTitle icon={Network} title="Danh mục BU & bộ lọc" />
              ...
              <AnimatePresence mode="wait">
                ...
                {tab === "matrix" && (
                  <motion.div key="mx" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                    ...
                    />
                    <div className="mt-2 text-xs text-neutral-500">Mẹo: Chỉ hiển thị các công ty thuộc <b>BU</b> của anh. Click ô để chat đúng ngữ cảnh hoặc tạo chỉ đạo.</div>
                  </motion.div>
                )}
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-neutral-200 bg-white p-4">
            <SectionTitle icon={Bell} title="Tín hiệu BU" />
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2"><TriangleAlert className="h-4 w-4" /><span>2 KPI BU dưới ngưỡng</span></div>
              <div className="flex items-center gap-2"><ClipboardCheck className="h-4 w-4" /><span>4 phiếu trình chờ duyệt</span></div>
              <div className="flex items-center gap-2"><CalendarDays className="h-4 w-4" /><span>Họp BU 16:00</span></div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <button title="Exec: ≤1B/phiếu; ≤5B/tháng" className="rounded-xl border px-3 py-2 hover:bg-neutral-50" onClick={() => alert("Phân bổ ngân sách")}>Phân bổ ngân sách</button>
              <button title="Exec: ≤1B/phiếu; ≤5B/tháng" className="rounded-xl border px-3 py-2 hover:bg-neutral-50" onClick={() => alert("Phê duyệt nhanh")}>Phê duyệt nhanh</button>
              <button className="rounded-xl border px-3 py-2 hover:bg-neutral-50" onClick={() => alert("Chỉ đạo xuống Director")}>Giao Director</button>
              <button className="rounded-xl border px-3 py-2 hover:bg-neutral-50" onClick={() => alert("Xuất báo cáo BU")}>Xuất báo cáo BU</button>
            </div>
          </div>
        </div>
      </div>
